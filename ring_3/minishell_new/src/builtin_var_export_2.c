...
/*   Updated: 2025/01/20 16:55:26 by jwhitley         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char **sort_2d_array_by_ascii(char **array)
{
    ...
    while (env_copy[i])
    {
        ...
        while (env_copy[j])
        {
            ...
            {
                temp = env_copy[i];
                //printf("temp = %s just copied from env_copy = %s\n", temp, env_copy[i]);
                env_copy[i] = env_copy[j];
                //printf("env_copy[i = %i] > %s == env_copy[j=%i] > %s\n",i,env_copy[i],j, env_copy[i]);
                env_copy[j] = temp;
            }
            ++j;
        }
